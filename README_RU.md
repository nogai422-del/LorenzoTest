...
## Telethon: рекомендуемый вход через QR

В актуальной сборке основной способ подключения Telethon — **Web Admin → Telethon → Создать QR-код**.

1. Сохраните API ID и API Hash.
2. Нажмите «Создать QR-код».
3. На телефоне с уже авторизованным Telegram откройте **Настройки → Устройства → Подключить устройство**.
4. Отсканируйте QR из Web Admin.
5. Если Telegram запросит 2FA, введите пароль в Web Admin.

Вход по номеру оставлен как резервный. После запроса панель показывает фактический канал доставки, который вернул Telegram (приложение Telegram, email, Fragment, SMS/звонок и т. д.), а также доступный следующий способ и таймаут повтора. Не нажимайте повторную отправку много раз подряд: Telegram применяет flood limits.
