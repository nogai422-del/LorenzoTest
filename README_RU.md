...
- Для каждого Telegram-администратора можно создать отдельный web-логин и пароль. Пароль хранится только как PBKDF2-SHA256 hash с индивидуальной солью.
- Смена пароля, отключение web-доступа или удаление администратора сразу инвалидирует его активные web-сессии.
- Управление web-учётками, Telethon и скрытыми чатами доступно только Owner.
...
После первого запуска войдите аварийной Owner-учёткой из `WEB_ADMIN_USERNAME` / `WEB_ADMIN_PASSWORD`. Затем откройте **Администраторы → Web-доступ администраторов** и создайте каждому администратору персональный логин и пароль.

После того как отдельная учётка Owner проверена, `WEB_ADMIN_PASSWORD` можно удалить из Bothost ENV — тогда аварийный общий Owner-вход отключится, а останутся только персональные web-логины.
...
Для первого входа владельца используйте длинный уникальный `WEB_ADMIN_PASSWORD`. После создания и проверки отдельной Owner web-учётки рекомендуется удалить `WEB_ADMIN_PASSWORD` из ENV и использовать только персональные логины.

У каждого администратора собственные credentials. Пароли не сохраняются в открытом виде и не выводятся обратно в интерфейс. При смене пароля увеличивается версия сессии, поэтому старые cookies перестают работать сразу.
...
## Исправление учёта участников

История сообщений больше не изменяет текущий статус членства. Текущий состав определяется только полной синхронизацией участников Telethon и realtime-событиями вступления/выхода. Авторы старых сообщений, уже вышедшие из группы, остаются в истории активности с is_active=0. Метрика «Новые 7д» считается только по joined_at.

## Owner-only: Telethon и скрытые чаты

В этой сборке инфраструктурные функции отделены от обычной админки:

- пункт **Telethon** в Telegram-админке показывается только `OWNER_ID`;
- обычный администратор не видит Telethon-статус в `/admin` и не может открыть callback Telethon вручную;
- web-маршруты `/telethon/*` помечены как owner-only;
- в Web Admin владелец может нажать **«Скрыть из панели»** у любого обнаруженного чата;
- скрытый чат автоматически снимается с отслеживания, для него выключаются алерты и отменяются ожидающие sync-задачи;
- скрытые чаты не попадают в Dashboard, списки участников/чатов, Telegram chat-picker и фоновые sync/alert циклы;
- повторное обнаружение чата через Bot API или Telethon не возвращает его в мониторинг;
- владелец видит архив в **Чаты → Скрытые чаты** и может восстановить чат. После восстановления отслеживание нужно включить вручную.

Текущая web-авторизация является owner-консолью: обычные Telegram-администраторы получают права через таблицу `system_admins`, но web-пароль владельца им передавать не нужно.

## Engagement / тесты / мобильная панель

В этой сборке добавлены дополнительные инструменты работы с активностью:

- Dashboard показывает отдельную метрику **«Ни разу не писали»**.
- В **Участники → Ни разу не писали** отображаются текущие участники с 0 сообщений за всё время наблюдения.
- В Telegram `/admin → Ни разу не писали` можно выбрать участника и отправить ему настроенный публичный пинг в группе.
- Текст пинга настраивается в **Web Admin → Бот → Персональный пинг участнику**. Поддерживаются `{mention}`, `{name}`, `{username}`, `{user_id}`.
- В **Web Admin → Бот → Тест оповещений** можно выбрать группу и администратора и отправить настоящий тестовый inactivity-alert. История реальных алертов при этом не меняется.
- В Telegram аналогичный тест доступен через `/admin → Оповещения → Тест оповещения мне`.
- Страницы Dashboard, Участники, Администраторы, Чаты, Бот и Telethon адаптированы под телефон; на узком экране используется отдельная нижняя навигация и карточки вместо широких таблиц.

### Добавление администратора по @username

В **Web Admin → Администраторы** достаточно ввести `@username`. Если Telethon подключён, Lorenzo получает актуальный Telegram User ID напрямую через MTProto. Если Telethon временно недоступен, может использоваться уже синхронизированный участник с таким username. В таблице прав всегда хранится числовой User ID, а не строковый username.

Если пользователя нет среди синхронизированных участников и Telethon не подключён, панель попросит сначала подключить Telethon либо использовать числовой Telegram ID.
